...
from tpu_beam_search.beam_collector import pallas_collector_append, reserve_group
...
def main():
    ...
    a = rng.integers(0,2**32,(8,8,256),dtype=np.uint32)
    b = rng.integers(0,2**32,a.shape,dtype=np.uint32)
    incoming = rng.integers(0,2**32,(8,8,128),dtype=np.uint32)
    ...
    for r,(used,busy,current,amount) in enumerate(scenarios):
        ...
        if r == 7:
            continue
        reservation = reserve_group(capacity=256,clean=used,dirty=(0,0),
    ...
    case = dict(input_sha256=digest(inputs),expected_sha256=digest(expected))
    report['cases'].append(case)
    save()
    print(json.dumps(report),flush=True)
    args = tuple(jax.device_put(x,sharding) for x in inputs)
    fn = jax.jit(jax.shard_map(local_append,mesh=mesh,in_specs=(p,)*5,out_specs=(p,)*3,check_vma=False))
