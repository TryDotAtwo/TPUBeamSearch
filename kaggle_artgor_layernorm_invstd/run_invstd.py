"""Run fixed-variance invstd attribution from one public source SHA."""
...
COMMIT_SHA = "df562624015a2b27b722e14915138d7345c0764b"
REPOSITORY = "https://github.com/TryDotAtwo/TPUBeamSearch.git"
CHECKOUT = Path("/tmp/TPUBeamSearch-artgor-layernorm-invstd")
OUTPUT = Path("/kaggle/working/artgor_layernorm_invstd")
...
def main() -> None:
    ...
    command = (
        sys.executable, "-m", "benchmarks.artgor_layernorm_invstd",
        "--output", str(OUTPUT),
    )
    with (OUTPUT / "invstd.log").open("w", encoding="utf-8") as log:
