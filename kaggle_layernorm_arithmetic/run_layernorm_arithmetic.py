"""Private Kaggle bootstrap. All benchmark code comes from one public Git SHA."""
...
COMMIT_SHA = "2e9602829b8e4fa8498b64461f64c556e77ad4f4"
REPOSITORY = "https://github.com/TryDotAtwo/TPUBeamSearch.git"
CHECKOUT = Path("/tmp/TPUBeamSearch")
OUTPUT = Path("/kaggle/working/arithmetic_ab")


def main():
    if not re.fullmatch(r"[0-9a-f]{40}", COMMIT_SHA):
        raise ValueError("runner requires a full source commit SHA")
    # JAX 0.10.2's published TPU extra requires libtpu 0.0.42.*.
    # Pin the patch too: do not silently upgrade to an unrelated libtpu release.
    ...
    command = (sys.executable, "-m", "benchmarks.stream1_layernorm_arithmetic", "--output", str(OUTPUT))
