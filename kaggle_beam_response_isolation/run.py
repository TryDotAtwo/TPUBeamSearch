"""Private response stage compilation isolation; no JAX in coordinator."""
import json
import os
...
COMMIT_SHA='6b8b4e28537e8d6681492d1fefd28132652a3cb6'
CHECKOUT=Path('/tmp/TPUBeamSearch-response-isolation')
OUTPUT=Path('/kaggle/working/beam_response_isolation')
...
def main():
    ...
    with (OUTPUT/'process.log').open('w') as log:
        child=subprocess.run((sys.executable,'-m','benchmarks.beam_response_isolation_bundle',
            '--output',str(OUTPUT/'stages')),cwd=CHECKOUT,env=env,
            stdout=log,stderr=subprocess.STDOUT,check=False)
    report['returncode']=child.returncode
    manifest.write_text(json.dumps(report,indent=2))
    if child.returncode:
        raise RuntimeError(f'isolation bundle returned {child.returncode}; see stage logs')
