"""Run the exact Artgor notebook publication gate from one public Git SHA."""
...
COMMIT_SHA = "2b99bdf5116f828a21d35b2c5910467f6ab039c2"
REPOSITORY = "https://github.com/TryDotAtwo/TPUBeamSearch.git"
CHECKOUT = Path("/tmp/TPUBeamSearch-artgor-exact-validation")
...
def main() -> None:
    ...
    command = (
        sys.executable,
        "-m",
        "benchmarks.artgor_exact_notebook_validation",
        "--output",
        str(OUTPUT),
    )
    with (OUTPUT / "validation.log").open(
        "w", encoding="utf-8"
    ) as log:
