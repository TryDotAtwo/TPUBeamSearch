"""The execution A/B launcher must pin one public source revision."""
import ast
import json
...
def test_execution_ab_launcher_is_private_tpu_and_reproducibly_pinned():
    folder = Path(__file__).resolve().parents[1] / "kaggle_inference_execution_ab"
    metadata = json.loads((folder / "kernel-metadata.json").read_text())
    assert metadata["id"] == "trydotatwo/tpu-inference-execution-ab"
    ...
    assert {
        ...
        "benchmarks.stream1_inference_execution_ab", "benchmark.log", "--detach",
        "PYTHONPATH", "PYTHONUNBUFFERED",
        "/kaggle/working/inference_execution_ab",
    } <= strings
